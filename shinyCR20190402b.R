...
server <- function(input, output) {
  ...
  cutOmar <- function (x, breaks, labels = NULL, include.lowest = FALSE, right = TRUE, 
  ...
  output$incMap <- renderLeaflet({
    ...
    #                          domain = min(regions2@data$rate, na.rm=T):max(regions2@data$rate, na.rm=T)
    #                          )
    ...
    leaflet() %>%
      ...
      addLegend("bottomright", 
                ...
                #labels = colorOrder$rate[order(-colorOrder$rate)],
                labels = rev(as.character(levels(cutOmar(colorOrder$rate[order(-colorOrder$rate)], 
                                                 breaks = 7, ordered_result = TRUE)))),
